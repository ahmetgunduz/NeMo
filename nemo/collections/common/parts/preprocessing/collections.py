...
class AudioText(_Collection):
    ...
    OUTPUT_TYPE = collections.namedtuple(
        ...
        field_names="id audio_file duration text_tokens offset text_raw speaker orig_sr",
    )
...
class CustomSpeechLabel(_Collection):
    """List of audio-label correspondence with preprocessing."""

    CUSTOM_OUTPUT_TYPE = collections.namedtuple(
        typename="SpeechLabelEntity",
        field_names="audio_file duration label offset wer",
    )
    ...
    def __init__(
        ...
        if index_by_file_id:
            self.mapping = {}
        output_type = self.CUSTOM_OUTPUT_TYPE
        data, duration_filtered = [], 0.0
        for audio_file, duration, command, offset, wer in zip(
            ...
            data.append(output_type(audio_file, duration, command, offset, wer))
        ...
        logging.info(
            "# {} files loaded accounting to # {} labels".format(
...
class FeatureSequenceLabel(_Collection):
    ...
    def __init__(
        ...
        logging.info(
            "# {} files loaded including # {} unique labels".format(
                len(data), len(self.uniq_labels)
            )
        )
        super().__init__(data)
...
class ASRFeatureSequenceLabel(FeatureSequenceLabel):
    ...
    def _parse_item(self, line: str, manifest_file: str) -> Dict[str, Any]:
        ...
        if "feature_filename" in item:
            ...
            raise ValueError(
                f"Manifest file has invalid json line "
                f"structure: {line} without proper feature file key."
            )
