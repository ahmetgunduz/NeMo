...
from nemo.collections.asr.data.audio_to_label import (
    ...
    CustomAudioToSpeechLabelDataset,
)
...
from nemo.collections.common.losses import CrossEntropyLoss as CELoss, CustomLoss
...
class EncDecSpeakerLabelModel(ModelPT, ExportableEncDecModel):
    ...
    * Speaker Decoder
    ...
    def extract_labels(data_layer_config):
        ...
        logging.warning(
            f"Total number of {len(labels)} found in all the manifest files."
        )
        return labels
    ...
    def setup_finetune_model(self, model_config: DictConfig):
        ...
        """
        logging.info(
            "Setting up data loaders with manifests provided from model_config"
        )
        ...
        if "train_ds" in model_config and model_config.train_ds is not None:
            ...
            raise KeyError(
                "train_ds is not found in model_config but you need it for fine tuning"
            )

        if self.labels is None or len(self.labels) == 0:
            raise ValueError(
                f"New labels must be non-empty list of labels. But I got: {self.labels}"
            )
        ...
        logging.info(
            f"Changed decoder output to # {self.decoder._num_classes} classes."
        )
        ...
        with open(self.test_manifest, "r") as manifest:
            for idx, line in enumerate(manifest.readlines()):
                ...
                if uniq_name in out_embeddings:
                    raise KeyError(
                        "Embeddings for label {} already present in emb dictionary".format(
                            uniq_name
                        )
                    )
...
class EncDecSpeechRegressionModel(ModelPT, ExportableEncDecModel):
    """Encoder decoder class for speaker label models.
    Model class creates training, validation methods for setting up data
    performing model forward pass.
    Expects config dict for
    ...
    * Speaker Decoder
    ...
    def __init__(self, cfg: DictConfig, trainer: Trainer = None):
        ...
        # else:
        #
        logging.info("Training with  Custom loss")

        ## TODO: New loss : (probs ** wer).mean(dim=x)
        self.loss = CustomLoss()

        self.task = None

        ## TODO: metric
        self._metric = TopKClassificationAccuracy(top_k=[1])
    ...
    def __setup_dataloader_from_config(self, config: Optional[Dict]):
        ...
        self.dataset = CustomAudioToSpeechLabelDataset(
            manifest_filepath=config["manifest_filepath"],
            labels=config["labels"],
            featurizer=featurizer,
            max_duration=config.get("max_duration", None),
        ...
        if self.task == "diarization":
            ...
            logging.info("Setting up regression parameters")
    ...
    # PTL-specific methods
    def training_step(self, batch, batch_idx):
        audio_signal, audio_signal_len, labels, wers = batch
        logits, _ = self.forward(
            input_signal=audio_signal, input_signal_length=audio_signal_len
        )
        loss = self.loss(logits=logits, wers=wers, labels=labels)
    ...
    def setup_finetune_model(self, model_config: DictConfig):
        ...
        if self.dataset.labels is None or len(self.dataset.labels) == 0:
            raise ValueError(
                f"New labels must be non-empty list of labels. But I got: {self.dataset.labels}"
            )
        ...
        if (
            scratch_labels == self.dataset.labels
        ):  # checking for new finetune dataset labels
            logging.warning(
                "Trained dataset labels are same as finetune dataset labels -- continuing change of decoder parameters"
            )
    ...
    @torch.no_grad()
    def get_logits(self, path2audio_file):
        audio, sr = librosa.load(path2audio_file, sr=None)
        target_sr = self._cfg.train_ds.get("sample_rate", 16000)
        if sr != target_sr:
            audio = librosa.core.resample(audio, sr, target_sr)
